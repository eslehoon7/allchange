  ...
  return (
    <div className="flex flex-col gap-4 w-full h-full">
      <div className="flex items-center justify-end px-2">
        <span className="text-[10.5px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded font-bold tracking-tight border border-slate-200/50">
          광고협찬 영역
        </span>
      </div>
        ...
        {ads.map((ad, idx) => {
          ...
          if (isClosed) {
            ...
            return (
              ...
              >
                <span>협찬 제안 슬롯 {idx + 1} 비활성화됨</span>
                <button
          ...
          return (
            <motion.div
              ...
              className="relative group cursor-pointer bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-[195px] overflow-hidden"
              style={{
                id: `ad-${position}-${idx}`,
              }}
            >
              ...
              <div className="flex flex-col gap-2 relative z-10">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-[10.5px] font-bold px-2 py-0.5 rounded text-white bg-gradient-to-r ${ad.accentColor} tracking-tight`}>
                      {ad.badge}
                    </span>
                    <span className="text-[10.5px] text-slate-500 font-bold bg-slate-100 border border-slate-200/40 px-2 py-0.5 rounded tracking-tight">
                      광고협찬
                    </span>
                  </div>
                  <button
                ...
                <span className="text-sm font-bold text-slate-800 leading-snug mt-1 block">
                  업체명:<br />협찬 대기중
                </span>
              ...
              <div className="flex items-center justify-end mt-auto pt-2 pb-1 relative z-10 border-t border-slate-100">
                <span className="text-[11px] font-semibold text-blue-600 flex items-center gap-1 group-hover:translate-x-1 transition-transform whitespace-nowrap">
                  자세히 알아보기
                  <ArrowRight size={12} />
                </span>
              </div>
      ...
      <div className="bg-blue-50/50 rounded-2xl p-3.5 border border-blue-100/60 flex items-center gap-2.5">
        <ShieldCheck className="text-blue-500 shrink-0" size={18} />
        <div className="flex flex-col min-w-0">
          <span className="text-[11px] sm:text-xs font-bold text-blue-900 whitespace-nowrap tracking-tight">
            100% 안전 보장
          </span>
          <span className="text-[9.5px] sm:text-[10px] text-blue-700/80 leading-tight tracking-tight mt-0.5 break-keep block">
            다운로드 직후<br />서버에서 파일 완전 청소
          </span>
        </div>
      </div>
    </div>
